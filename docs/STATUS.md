...
| **E. Evaluation** | |
| Attack success / task success / false reject / etc. | ✅ R_bad, R_bad_outcome, R_forge, Delta_auth, success_rate, bootstrap CIs |
...
| 5. Certificate gating | ✅ Taint detection + certificate validation + verifier in run loop |
...
| **verifier/** | ✅ `certificate.py` (make + validate), `taint.py` (n-gram overlap), `verifier.py` (verify_with_debug) |
| **defense/** | ✅ `allowlist.py`, `taskshield.py`, `judge.py`, `intentguard.py`, `prompt.py` |
| **eval/** | ✅ `metrics.py` (is_bad_action, compute metrics), `bootstrap.py`, `plots.py` |
